# Agent Work Log

이 파일은 Gemini CLI 에이전트가 수행한 모든 작업 내용을 날짜별로 기록하는 로그입니다.

## 2026-05-28

### 🛠 주요 작업 내용

1.  **Node.js 환경 업데이트 및 고정**
    *   `react-doctor` 호환성 문제(v22.12.0 이상 필요)를 해결하기 위해 Node.js를 `v22.22.3`으로 업데이트.
    *   `.nvmrc` 파일을 생성하여 프로젝트의 Node 버전을 고정함.

2. **React Doctor 진단 및 코드 최적화 (100점 달성)**
    *   진단 결과를 바탕으로 3단계에 걸친 대규모 최적화 수행:
        *   **[Phase 1] 컴포넌트 구조 개선**: 434라인의 거대 컴포넌트(`MafiaGame`)를 8개의 독립적인 서브 컴포넌트로 분리.
        *   **[Phase 2] 접근성 및 SEO**: 페이지 메타데이터 추가, `aria-label` 보완, `new Date()` Hydration 에러 수정.
        *   **[Phase 3] 성능 최적화**: Tailwind CSS 최신 축약 문법 적용, 비동기 작업 병렬화(`Promise.all`), 불필요한 빌드 파일 정리.
    *   **결과**: React Doctor 최종 점수 **100/100** 달성. (Dead Code 체크 제외 설정 적용)

3.  **프로젝트 구조 정리 및 기록 관리 강화**
    *   사용되지 않는 빌드 결과물(`docs-site/build`) 삭제 및 `.gitignore` 설정 확인.
    *   `react-doctor.config.json`을 추가하여 프로젝트 전용 검사 규칙 설정.
    *   **과거 기술 이력 복원**: `CHANGELOG_INTERNAL.md`에서 누락되었던 5월 26일의 대규모 기술적 변경 내역(경험치 시스템, 데이터 영속성 등)을 Git 로그 기반으로 분석하여 복원함.
    *   **기록 규칙 명문화**: `AGENTS.md`를 업데이트하여 향후 모든 에이전트가 `AGENT_WORK_LOG.md`와 `CHANGELOG_INTERNAL.md`를 필수적으로 유지 관리하도록 지침을 강화함.


4.  **커밋 관리 체계 준수**
    *   기존 수동 커밋을 취소하고, 프로젝트 전용 커밋 스크립트(`shared/commit-script.ts`)의 규칙에 맞춰 원자적(Atomic) 커밋으로 재구성함.

### 📝 비고
- 마지막 커밋 메시지 인코딩 이슈 발생 확인 및 수정 시도.
- 향후 모든 작업 내용은 이 파일에 지속적으로 기록 예정.

## 2026-05-27

### 🛠 주요 작업 내용

1.  **튜토리얼 및 사용자 경험 개선**
    *   신규 사용자를 위한 5단계 인터랙티브 튜토리얼 시스템 구현.
    *   `tutorialStep` 상태를 도입하여 게임의 전반적인 흐름을 안내하는 모달 가이드 추가.

2.  **NPC 상호작용 및 생동감 강화**
    *   `useMafiaGame.ts` 내 NPC 대화 및 투표 로직을 고도화하여 게임 플레이 시 생동감 부여.

3.  **프로젝트 문서 및 로드맵 관리**
    *   `roadmap.md` 및 `changelog.md` 업데이트를 통해 향후 개발 방향 및 변경 이력 동기화.
