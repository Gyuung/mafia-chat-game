...
### 기능 구현 & UX
- **인터랙티브 튜토리얼 시스템**: 
  - 신규 사용자를 위한 5단계 가이드 모달 구현 및 상태 관리 로직 추가.
  - `tutorialStep` 상태를 통한 단계별 UI 강조 및 흐름 제어.
- **NPC 상호작용 고도화**: 
  - 대화 및 투표 시 NPC들의 반응 로직 개선을 통해 게임의 생동감 부여.

### 문서화 & 로드맵
- **프로젝트 로드맵 업데이트**: `roadmap.md`에 향후 기술적 목표(멀티플레이어 기반 마련 등) 반영.
- **세션 노트 동기화**: `SESSION_NOTES.md`를 최신화하여 작업 연속성 유지.
